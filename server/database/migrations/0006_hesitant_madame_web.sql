ALTER TABLE `order_items` RENAME TO `orders_items`;--> statement-breakpoint
...
CREATE TABLE `__new_orders_items` (
	...
	`created_at` integer DEFAULT '"2025-01-28T10:50:07.827Z"' NOT NULL,
	FOREIGN KEY (`order_id`) REFERENCES `orders`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`item_id`) REFERENCES `items`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
INSERT INTO `__new_orders_items`("id", "order_id", "item_id", "quantity", "price", "created_at") SELECT "id", "order_id", "item_id", "quantity", "price", "created_at" FROM `orders_items`;--> statement-breakpoint
...
CREATE TABLE `__new_coupons` (
	...
	`created_at` integer DEFAULT '"2025-01-28T10:50:07.826Z"' NOT NULL,
	FOREIGN KEY (`created_by_id`) REFERENCES `users`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
INSERT INTO `__new_coupons`("id", "code", "name", "is_active", "created_by_id", "created_at") SELECT "id", "code", "name", "is_active", "created_by_id", "created_at" FROM `coupons`;--> statement-breakpoint
...
CREATE TABLE `__new_users` (
	...
	`password` text,
	`avatar` text,
	`created_at` integer DEFAULT '"2025-01-28T10:50:07.825Z"' NOT NULL
);
--> statement-breakpoint
